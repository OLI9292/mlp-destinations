...
class FrontCover extends Component {
  ...
  render() {
    ...
    return (
      ...
      >
        <Burger
          src={require(`../../Library/Images/${
            this.state.displayMobileMenu ? "exit.png" : "burger.png"
          }`)}
          onClick={() =>
        ...
        <Mask
          isContact={path.includes("contact")}
          darkened={this.props.darkened}
        />

        <InnerContainer>
          <LogoContainer
            hide={path.includes("destinations/") || path.includes("contact")}
          >
            <span
              style={{
          ...
          <LinksContainer>
            <Link
              flex={1}
              onClick={() => this.setState({ redirect: "/services" })}
            >
              SERVICES
              <img
                alt={"underline"}
                src={underline}
                style={{ display: path.includes("services") ? "" : "none" }}
              />
            </Link>

            <Link
              flex={1.2}
              onClick={() => this.setState({ redirect: "/destinations" })}
            >
              DESTINATIONS
              <img
                alt={"underline"}
                src={underline}
                style={{ display: path.includes("destinations") ? "" : "none" }}
              />
            </Link>
            ...
            >
              ...
              <img
                alt={"underline"}
                src={underline}
                style={{ display: path.includes("about") ? "" : "none" }}
              />
            </Link>

            <Link
              flex={1}
              onClick={() => this.setState({ redirect: "/stories" })}
            >
              STORIES
              <img
                alt={"underline"}
                src={underline}
                style={{ display: path.includes("stories") ? "" : "none" }}
              />
            </Link>
            ...
            >
              ...
              <img
                alt={"underline"}
                src={underline}
                style={{ display: path.includes("contact") ? "" : "none" }}
              />
            </Link>
          </LinksContainer>
        </InnerContainer>
        ...
        {this.props.destination ? (
          ...
          >
            <h3
              style={{ fontFamily: "ATSackersGothicMedium", fontSize: "1.3em" }}
            >
